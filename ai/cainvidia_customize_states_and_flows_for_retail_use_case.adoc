...
summary:  
---
= Personalizar Estados e fluxos para casos de uso de varejo
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Você pode personalizar Estados e fluxos do Dialog Manager para seus casos de uso específicos. Em nosso exemplo de varejo, temos os quatro arquivos yaml a seguir para direcionar a conversa de acordo com diferentes intenções.

A seguinte lista de nomes de arquivo e descrição de cada arquivo:

* `main_flow.yml`: Define os fluxos e estados da conversa principal e direciona o fluxo para os outros três arquivos yaml quando necessário.
* `retail_flow.yml`: Contém estados relacionados com questões de varejo ou pontos de interesse. O sistema fornece as informações da loja mais próxima ou o preço de um determinado item.
* `weather_flow.yml`: Contém estados relacionados com questões meteorológicas. Se a localização não puder ser determinada, o sistema faz uma pergunta de acompanhamento para esclarecer.
* `error_flow.yml`: Lida com casos em que as intenções do usuário não se enquadram nos três arquivos yaml acima. Depois de exibir uma mensagem de erro, o sistema redireciona para aceitar perguntas do usuário.as seções a seguir contêm as definições detalhadas para esses arquivos yaml.
