...
summary: Esta seção aborda o procedimento passo a passo para migrar dados CIFS com informações de segurança de uma origem para um sistema ONTAP de destino. 
---
= Migração de dados CIFS com ACLs de uma caixa de armazenamento de origem para o ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Esta seção aborda o procedimento passo a passo para migrar dados CIFS com informações de segurança de uma origem para um sistema ONTAP de destino.
...
NOTE: Se não houver um agregado de dados, crie um novo usando o `storage aggr create` comando.
...
....
. Crie um LIF de dados para atender às solicitações de clientes SMB.
+
....
...
2 entries were displayed.
....
. Montar o volume de dados de destino no namespace SVM.
+
....
...
....
. Inicie o serviço CIFS no SVM de destino.
+
....
...
....
. Verifique se a política de exportação padrão é aplicada ao SVM de destino.
+
....
...
....
. Modifique as regras de política de exportação para permitir acesso a clientes CIFS.
+
....
...
2 entries were displayed.
....
. Conete-se ao sistema cliente Windows onde o XCP está instalado. Navegue até o caminho de instalação do XCP.
+
....
C:\WRSHDNT>dir c:\netapp\xcp
dir c:\netapp\xcp
...
....
. Consulte as exportações SMB do nó de origem executando o `xcp show` comando no sistema host cliente do XCP Windows.
+
....
...
....
. Execute o `help` comando para copiar.
+
....
...
....
. No sistema ONTAP de destino, obtenha a lista de nomes de usuários locais e grupos locais que você precisa fornecer como valores para o `fallback-user` caminho e `fallback-group` argumentos.
+
....
...
5 entries were displayed
....
. Para migrar os dados CIFS com ACLs da origem para o destino, execute o `xcp copy` comando com `-acl` as opções e `–fallback-user/group` .
+
Para as `fallback-user/group` opções, especifique qualquer usuário ou grupo que possa ser encontrado no ative Directory ou usuário/grupo local para o sistema de destino.
...
C:\WRSHDNT>
....
. Se `xcp copy` resultar na mensagem de `ERROR failed to obtain fallback security principal` erro , adicione a caixa de destino no arquivo hosts (`C:\Windows\System32\drivers\etc\hosts`).
+
Use o seguinte formato para a entrada da caixa de destino de armazenamento NetApp.
...
....
. Se você ainda receber a mensagem de erro `ERROR failed to obtain fallback security principal` depois de adicionar a entrada da caixa de destino nos arquivos hosts, o usuário/grupo não existe no sistema de destino.
+
....
...
C:\WRSHDNT>
....
.  `xcp copy`Use para migrar dados CIFS com ACLs (com ou sem a pasta raiz).
+
Sem a pasta raiz, execute os seguintes comandos:
...
....
+
Com a pasta raiz, execute os seguintes comandos:
