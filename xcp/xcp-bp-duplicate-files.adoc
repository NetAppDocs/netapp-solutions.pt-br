...
summary: O NetApp recebeu uma solicitação para localizar arquivos duplicados de um único volume ou vários volumes. A NetApp forneceu a seguinte solução. 
---
= Arquivos duplicados
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
O NetApp recebeu uma solicitação para localizar arquivos duplicados de um único volume ou vários volumes. A NetApp forneceu a seguinte solução.

Para um único volume, execute os seguintes comandos:
...
....
Para vários volumes, execute os seguintes comandos:
