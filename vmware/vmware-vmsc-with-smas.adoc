...
* link:vmware_vcf_asa_supp_mgmt_iscsi.html#deployment-steps["Hosts do VMware vSphere implantados com malha de armazenamento dupla (dois HBAs ou VLAN dupla para iSCSI) por host"].
...
* link:vmware_vcf_asa_supp_mgmt_iscsi.html#deployment-steps["VM de storage e LIFs estão disponíveis"]
