...
A NetApp tem vários sistemas de storage perfeitos para data centers empresariais e implantações de nuvem híbrida. O portfólio do NetApp inclui sistemas de armazenamento NetApp ONTAP, Cloud Volumes ONTAP, Cloud Volumes Service, Azure NetApp Files, FSX ONTAP para NetApp ONTAP, todos eles podem fornecer armazenamento persistente para aplicações em contêiner.
